...
9. **litellm is exact-pinned** (`litellm==1.100.1` in `backend/pyproject.toml`), not given a floor/range. As of 1.100.1, litellm ships prebuilt wheels for Python 3.14 (`cp310-abi3`, `requires-python <3.15,>=3.10`) and no longer exact-pins its own dependency tree — pydantic/httpx/python-dotenv/etc. are plain ranges compatible with this project's floors, so there is no corresponding `dependabot.yml` ignore list to maintain. (Earlier, 1.83.7 was the newest release with Python 3.14 wheel support — 1.92.0+ dropped 3.14 wheels and required a Rust toolchain to build from sdist — and 1.83.7 did exact-pin its sub-deps, which is why that ignore list existed. Both constraints no longer apply at 1.100.1; re-verify PyPI metadata per rule 10 before assuming this still holds on a future bump, since litellm's packaging approach has changed at least once already.)
